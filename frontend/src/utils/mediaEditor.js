...
const getFilter = (filterId) => MEDIA_FILTERS.find(filter => filter.id === filterId) || MEDIA_FILTERS[0];

export const getDefaultMediaEdit = () => ({
  filter: 'original',
  rotate: 0,
  flipX: false
});

export const normalizeMediaEdit = (edit = {}) => ({
  filter: getFilter(edit.filter).id,
  rotate: (((Number(edit.rotate) || 0) % 360) + 360) % 360,
  flipX: Boolean(edit.flipX)
});

export const hasMediaEdits = (edit = {}) => {
  const normalized = normalizeMediaEdit(edit);
  return normalized.filter !== 'original' || normalized.rotate !== 0 || normalized.flipX;
};

export const getMediaEditPreviewStyle = (edit = {}) => {
  const normalized = normalizeMediaEdit(edit);
  return {
...
export const applyImageEdits = async (file, edit = {}) => {
  const normalized = normalizeMediaEdit(edit);
