...
export default function GameOverModal({
  ...
  return (
    <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-sm overflow-hidden rounded-3xl border border-white/10 bg-gray-950 text-white shadow-2xl shadow-pink-500/20">
        <div className="relative p-5 text-center">
          <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-cyan-300 via-pink-500 to-yellow-300" />
          <div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-gradient-to-br from-cyan-400 to-pink-500 shadow-xl shadow-pink-500/25">
            <Trophy size={30} />
          </div>
          <h3 className="mt-4 text-2xl font-black tracking-normal">{title}</h3>
          <p className="mt-2 text-sm font-semibold text-white/55">{detail || 'Nice run. Try again or exit back to the game panel.'}</p>
