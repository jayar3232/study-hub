...
const getEntityId = (entity: PresenceUser | string | null | undefined): string =>
  String((typeof entity === 'string' ? entity : entity?._id || entity?.id) || '');

type OnlineRosterProps = {
  compact?: boolean;
  limit?: number;
  title?: string;
};

export default function OnlineRoster({ compact = false, limit = 10, title = 'Online now' }: OnlineRosterProps) {
  const { onlinePeople, hasStory } = usePresence();
  const [profileUser, setProfileUser] = useState<PresenceUser | null>(null);
  const visiblePeople = (onlinePeople as PresenceUser[]).slice(0, limit);
      ...
      {visiblePeople.length === 0 ? (
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {visiblePeople.map((person) => {
            const avatar = resolveMediaUrl(String(person.avatar || ''));
