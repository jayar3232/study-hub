...
export default function Dashboard() {
  ...
  const renderStoryPanel = ({ mobile = false } = {}) => (
    <section className={`dashboard-story-panel ${mobile ? 'dashboard-story-panel--fb-mobile' : ''} rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/55 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20`}>
      {!mobile && (
      ...
      <div className="dashboard-story-rail flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <button
          type="button"
          onClick={mobile ? openStoryPicker : () => navigate('/profile')}
          disabled={mobile && storyUploading}
          className="dashboard-create-story-card flex h-40 w-28 shrink-0 flex-col justify-end overflow-hidden rounded-2xl border border-blue-100 bg-blue-50 p-3 text-left text-blue-800 dark:border-blue-900/50 dark:bg-blue-950/25 dark:text-blue-100"
        >
          {mobile && (
            ...
            <div className="space-y-2">
              ...
              {!gameLeaders.length && (
                <p className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center text-sm font-semibold text-slate-500 dark:border-slate-800 dark:bg-slate-950/70 dark:text-slate-400">
                  Play Game Hub games to show ranks here.
                </p>
              )}
            </div>
          </Panel>
        ...
        <div className="story-draft-overlay" onClick={closeStoryDraft}>
          <section className="story-draft-sheet" onClick={event => event.stopPropagation()} aria-label="Create story">
            ...
            <div className="grid gap-4 p-4 md:grid-cols-[minmax(0,1fr)_18rem]">
              <div className="story-draft-preview">
                {activeStoryDraftItem?.fileType === 'video' ? (
                  <video src={activeStoryDraftItem.previewUrl} controls playsInline className="h-full max-h-[70vh] w-full rounded-2xl bg-black object-contain" />
                ) : (
                  <img
